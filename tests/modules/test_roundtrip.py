...
Dogfoods the full lifecycle against the real modules/bridge/ first-party
module: install -> CLAUDE.md contains <module name="bridge"> -> uninstall ->
...
BRIDGE_DIR = MODULES_ROOT / "bridge"
...
class TestBridgeDogfood:
    """Install + uninstall the real first-party bridge module end-to-end.

    Uses the actual ``modules/bridge/`` directory in the repo — this is the
    Phase 3 dogfood proving the entire module system works on a real module.
    """

    def test_bridge_install_writes_claude_md_with_module_section(
        ...
        assert mods.get_entry(tmp_hub_dir, "bridge") is not None
        assert mods.list_installed(tmp_hub_dir) == ["bridge"]
        ...
        # Uninstall
        mods.uninstall("bridge", tmp_hub_dir, BRIDGE_DIR)
        assert mods.get_entry(tmp_hub_dir, "bridge") is None
        assert mods.list_installed(tmp_hub_dir) == []
    ...
    def test_bridge_owned_paths_empty_means_no_hub_leakage(
        self, tmp_hub_dir: Path
    ) -> None:
    ...
    def test_bridge_reinstall_rejected(self, tmp_hub_dir: Path) -> None:
